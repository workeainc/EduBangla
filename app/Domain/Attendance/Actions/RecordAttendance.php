...
class RecordAttendance
{
    public function handle(AttendanceSession $session, array $rows, int $actorId): void
    {
        DB::transaction(function () use ($session, $rows, $actorId) {
            ...
            $validated[] = ['school_id' => $session->school_id, 'attendance_session_id' => $session->id, 'student_id' => $row['student_id'], 'enrollment_id' => $enrollment->id, 'status' => $row['status'], 'recorded_at' => now(), 'recorded_by' => $actorId, 'remarks' => $row['remarks'] ?? null];
            }
            foreach ($validated as $row) {
                StudentAttendance::updateOrCreate(
                    ['attendance_session_id' => $row['attendance_session_id'], 'student_id' => $row['student_id']],
                    $row,
                );
            }
        });
